...
class Block(nn.Module):
    def __init__(self, config, vis, use_adapter=False):
        ...
        if use_adapter == True:
            self.adapterlayer = AdapterBlock(config.hidden_size,adapter_hidden_dim=32,dropout=0.0)
    def forward(self, x):
        h = x
...
class Encoder(nn.Module):
    def __init__(self, config, vis, use_adapter=False):
        ...
        for i in range(config.transformer["num_layers"]):
            if use_adapter == True and i%12 == 0:
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis, use_adapter=False):
